...
namespace JEAD
{
    public class Baralho
    {
        ...
        {
            //ESSE METODO EMBARALHA AS CARTAS QUE ESTAO NA LISTA E RETORNA A LISTA EMBARALHADA
            Random embaralhar = new Random();
            var cartas = Cartas.OrderBy(x => embaralhar.Next());
            return cartas.ToList();

        }

        public Carta PegarCartaJogador1()
        {
            //ESSE METODO RETORNA A PRIMEIRA CARTA DA LISTA E EM SEGUIDA A REMOVE REMOVE
            List<Carta> cartas = Embaralhar();
            cartas.RemoveAt(0);
            return cartas.FirstOrDefault();
        }       

        public Carta PegarCartaJogador2()
        {         
            //ESSE METODO RETORNA A PRIMEIRA CARTA DO RESTANTE DA LISTA SEM A PRIMEIRA CARTA RETIRADA DO METODO ANTERIOR   
            List<Carta> cartas = Embaralhar();          
            return cartas.FirstOrDefault();
        }      
