...
namespace RetaguardaESilva.Persistence.Persistencias
{
    public class FornecedorPersist : IFornecedorPersist
    {
        ...
        {
            return await _context.Fornecedor.AsNoTracking().Where(f => f.EmpresaId == empresaId).OrderBy(f => f.Id).ToListAsync();            
        }  

        public async Task<Fornecedor> GetFornecedorByIdAsync(int empresaId, int fornecedorId)
        {
            return await _context.Fornecedor.AsNoTracking().Where(f => f.Id == fornecedorId && f.EmpresaId == empresaId).OrderBy(f => f.Id).FirstOrDefaultAsync();
        }
